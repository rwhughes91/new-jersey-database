...
const Mutation = {
  updateLienStatus: async (parent, {
    ...
    const updatedLien = await _lien.default.findOneAndUpdate({
      lien_id
    }, {
      status
    }, {
  ...
  updateSubAmount: async (parent, {
    ...
    if (amount === 0) {
      lien.subs = lien.subs.filter(sub => sub.sub_type !== type || sub.sub_date.getTime() !== date);
      message = 'Deleted';
    } else {
      const subIndex = lien.subs.findIndex(sub => sub.sub_type === type && sub.sub_date.getTime() === date);
